...
export default function WhatsAppFloat() {
  ...
  useEffect(() => {
    ...
    const pulseTimer = setTimeout(() => setPulseActive(false), 8000);
    return () => {
      clearTimeout(timer);
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex items-end gap-3">
      ...
      <a
        ...
        aria-label="Falar com a CanaBene pelo WhatsApp"
        className="relative w-16 h-16 bg-whatsapp-green hover:bg-brand-primary text-white rounded-full shadow-2xl shadow-brand-primary/40 flex items-center justify-center transition-all duration-300 hover:scale-110 group"
      >
        {/* Pulse ring */}
        {pulseActive && (
          <span className="absolute inset-0 rounded-full bg-whatsapp-green opacity-40 animate-ping" />
        )}
